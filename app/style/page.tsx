    ...
    {!searched ? <>
      ...
      <div className="card searchBox styleSearch">
        ...
        {filtersOpen && <div className="filterPanel premiumFilters">
          ...
          <details className="filterDrop"><summary>Sizes · {[topSize !== 'Any' ? 'Top ' + topSize : '', bottomSize !== 'Any' ? 'Bottom ' + bottomSize : '', shoeSize !== 'Any' ? 'Shoe ' + shoeSize : ''].filter(Boolean).join(', ') || 'Any'}</summary><div className="sizeFilterPanel"><div className="sizeFilterGroup"><h4>Top size</h4><div className="sizeScroller">{topSizes.map(s=><button type="button" key={'top-'+s} className={topSize === s ? 'sizeChip active' : 'sizeChip'} onClick={()=>setTopSize(s)}>{s}</button>)}</div></div><div className="sizeFilterGroup"><h4>Bottom size</h4><div className="sizeScroller">{bottomSizes.map(s=><button type="button" key={'bottom-'+s} className={bottomSize === s ? 'sizeChip active' : 'sizeChip'} onClick={()=>setBottomSize(s)}>{s}</button>)}</div></div><div className="sizeFilterGroup"><h4>Shoe size</h4><div className="sizeScroller">{shoeSizes.map(s=><button type="button" key={'shoe-'+s} className={shoeSize === s ? 'sizeChip active' : 'sizeChip'} onClick={()=>setShoeSize(s)}>{s}</button>)}</div></div></div></details>
