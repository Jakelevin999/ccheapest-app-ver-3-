...
function cheapTrust(source='',link=''){const t=(source+link).toLowerCase();if(/amazon|walmart|target|nike|apple|bestbuy|zara|hm.com|uniqlo|asos|nordstrom|macys|gap|urbanoutfitters|stockx|goat/.test(t))return 9;if(/ebay|depop|poshmark|mercari/.test(t))return 7;return 6;}
function isAbsoluteUrl(link=''){return /^https?:\/\//i.test(link);}
function safeFallbackLink(title='', query=''){return `https://www.google.com/search?tbm=shop&q=${encodeURIComponent(title || query || 'clothing')}`;}
...
export async function POST(req:Request){
  const {description,url,page,mode}=await req.json();
  let query='';
  ...
  if(description && description.trim().length>2){
    ...
    try{const u=new URL(url);query=cleanText(u.pathname.replace(/[-_/]/g,' '));}catch{query=url;}
  }
